...
class SQLiteStore:
    ...
    async def list_services_for_status(self, user: UserInfo) -> List[sqlite3.Row]:
        """
        Returns rows: service_id, service_name, total_visible, taken_visible.
        """
        async with self._lock:
            return await asyncio.to_thread(self._list_services_for_status_sync, user)

    def _list_services_for_status_sync(self, user: UserInfo) -> List[sqlite3.Row]:
        type_filter: Optional[str] = None
        if not (user.is_admin or user.role in (ROLE_QA, ROLE_OBSERVER, ROLE_ADMIN)):
            if user.role == ROLE_BACKEND:
                type_filter = STAND_BACKEND
            elif user.role == ROLE_FRONTEND:
                type_filter = STAND_FRONTEND
        ...
        SELECT
          sv.id AS service_id,
          sv.name AS service_name,
          COUNT(s.id) AS total_visible,
          SUM(CASE WHEN su.id IS NOT NULL THEN 1 ELSE 0 END) AS taken_visible
        FROM services sv
        JOIN stands s ON s.service_id = sv.id
        LEFT JOIN stand_usage su ON su.stand_id = s.id AND su.released_at IS NULL
        """
        params: List[Any] = []
        if type_filter:
            sql += " WHERE s.type = ?"
            params.append(type_filter)
        sql += " GROUP BY sv.id, sv.name ORDER BY sv.name"
        return self._con.execute(sql, params).fetchall()

    async def list_stands_by_service_paginated(
        self, user: UserInfo, service_id: int, page: int, page_size: int
    ) -> Tuple[List[sqlite3.Row], int]:
        """
        Returns (rows, total_count) for given service (with visibility filter) and pagination.
        """
        async with self._lock:
            return await asyncio.to_thread(self._list_stands_by_service_paginated_sync, user, service_id, page, page_size)

    def _list_stands_by_service_paginated_sync(
        self, user: UserInfo, service_id: int, page: int, page_size: int
    ) -> Tuple[List[sqlite3.Row], int]:
        type_filter: Optional[str] = None
        if not (user.is_admin or user.role in (ROLE_QA, ROLE_OBSERVER, ROLE_ADMIN)):
        ...
        where = "WHERE s.service_id = ?"
        params: List[Any] = [service_id]
        if type_filter:
            where += " AND s.type = ?"
            params.append(type_filter)

        total = self._con.execute(f"SELECT COUNT(*) AS c FROM stands s {where}", params).fetchone()
        ...
        LIMIT ? OFFSET ?
        """
        rows = self._con.execute(sql, params + [page_size, offset]).fetchall()
        return (rows, total_count)

    async def search_stands_paginated(
        self, user: UserInfo, query: str, page: int, page_size: int
    ) -> Tuple[List[sqlite3.Row], int]:
        """
        Searches stands by partial match in stand name or service name.
        Returns (rows, total_count) with visibility filter and pagination.
        """
        async with self._lock:
            return await asyncio.to_thread(self._search_stands_paginated_sync, user, query, page, page_size)

    def _search_stands_paginated_sync(
        self, user: UserInfo, query: str, page: int, page_size: int
    ) -> Tuple[List[sqlite3.Row], int]:
        q = f"%{query.strip()}%"
        type_filter: Optional[str] = None
        ...
        where = "WHERE (s.name LIKE ? OR sv.name LIKE ?)"
        params: List[Any] = [q, q]
        if type_filter:
            where += " AND s.type = ?"
            params.append(type_filter)
        ...
        total = self._con.execute(
            f"SELECT COUNT(*) AS c FROM stands s JOIN services sv ON sv.id=s.service_id {where}",
            params,
        ).fetchone()
        total_count = int(total["c"]) if total else 0
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _send_services_menu_message(update, context, user=user)


async def _clear_status_view_messages(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Deletes previously sent stand list messages for navigation/search.
    Best-effort: ignores errors.
    """
    if update.effective_chat is None:
        return
    msg_ids = context.user_data.get("status_view_message_ids")
    ...
    for mid in msg_ids:
        ...
        try:
            ...
            pass
    context.user_data["status_view_message_ids"] = []


async def _send_stands_page(
    update: Update,
    ...
    *,
    ...
    await _clear_status_view_messages(update, context)
    context.user_data["status_view_message_ids"] = []

    total_pages = max(1, (total_count + STATUS_PAGE_SIZE - 1) // STATUS_PAGE_SIZE)
    page = max(0, min(page, total_pages - 1))
    header = f"<b>{title}</b>\nСтраница: {page + 1}/{total_pages} • Стендов: {total_count}"
    nav_buttons: List[InlineKeyboardButton] = [InlineKeyboardButton("⬅️ Назад", callback_data=back_cb)]
    if page > 0:
        nav_buttons.insert(0, InlineKeyboardButton("◀️", callback_data=prev_cb))
    if page < total_pages - 1:
        nav_buttons.append(InlineKeyboardButton("▶️", callback_data=next_cb))
    ...
    context.user_data["status_view_message_ids"].append(nav_msg.message_id)
    ...
    for r in rows:
        ...
        context.user_data["status_view_message_ids"].append(msg.message_id)


async def _send_services_menu_message(update: Update, context: ContextTypes.DEFAULT_TYPE, *, user: UserInfo) -> None:
    store: SQLiteStore = context.application.bot_data["store"]
    services = await store.list_services_for_status(user)
    if not services:
        if update.effective_chat:
            await context.bot.send_message(chat_id=update.effective_chat.id, text="Нет доступных микросервисов/стендов.")
        return

    kb_rows: List[List[InlineKeyboardButton]] = []
    for s in services:
        sid = int(s["service_id"])
        name = str(s["service_name"])
        total = int(s["total_visible"] or 0)
        taken = int(s["taken_visible"] or 0)
        kb_rows.append([InlineKeyboardButton(f"{name} ({taken}/{total})", callback_data=f"status:svc:{sid}:p:0")])
    kb_rows.append([InlineKeyboardButton("🔎 Поиск стенда", callback_data="status:search")])
    ...
    text = (
        ...
        "Выберите микросервис (показано: занято/всего):"
    )
...
async def on_status_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    user_row = await store.get_user(update.effective_user.id)
    if not user_row:
        await _safe_edit_message(update, "Сначала зарегистрируйтесь через /start.", None)
        return
    ...
    if data in {"status:back", "status:menu"}:
        await _clear_status_view_messages(update, context)
        await _send_services_menu_message(update, context, user=user)
        return

    if data == "status:search":
        context.user_data["search_pending"] = True
        await _safe_edit_message(update, "Введите текст для поиска стенда (можно часть названия):", None)
        return

    # status:svc:<id>:p:<page>
    parts = data.split(":")
    if len(parts) != 5:
        return
    _, kind, sid_raw, pkey, page_raw = parts
    if kind != "svc" or pkey != "p":
        return
    try:
    ...
    rows, total = await store.list_stands_by_service_paginated(user, service_id, page, STATUS_PAGE_SIZE)
    title = f"Микросервис: {service_id}"
    # Try to show readable name if possible: we can infer from first row; if empty fallback.
    if rows:
        title = f"Микросервис: <b>{rows[0]['service_name']}</b>"

    await _send_stands_page(
        update,
        context,
        user=user,
        title=title,
        ...
        total_count=total,
        back_cb="status:menu",
        prev_cb=f"status:svc:{service_id}:p:{page-1}",
        next_cb=f"status:svc:{service_id}:p:{page+1}",
    )


async def on_search_nav_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Callback navigation for search results pages.
    """
    store: SQLiteStore = context.application.bot_data["store"]
    admin_ids: set[int] = context.application.bot_data["admin_ids"]
    if update.callback_query is None or update.effective_user is None:
    ...
    query = str(context.user_data.get("last_search_query") or "").strip()
    if not query:
        await _safe_edit_message(update, "Поиск не активен. Откройте /status и нажмите «Поиск».", None)
        return
    ...
    rows, total = await store.search_stands_paginated(user, query, page, STATUS_PAGE_SIZE)
    await _send_stands_page(
        update,
        context,
        user=user,
        title=f"Поиск: <b>{query}</b>",
        rows=rows,
        page=page,
        total_count=total,
        back_cb="status:menu",
        prev_cb=f"search:p:{page-1}",
        next_cb=f"search:p:{page+1}",
    )


async def cmd_search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /search <text> — search stands by name
    """
    store: SQLiteStore = context.application.bot_data["store"]
    admin_ids: set[int] = context.application.bot_data["admin_ids"]
    if update.effective_user is None or update.message is None:
    ...
    if not query:
        await update.message.reply_text("Использование: /search <часть названия стенда>")
        return
    context.user_data["last_search_query"] = query

    rows, total = await store.search_stands_paginated(user, query, 0, STATUS_PAGE_SIZE)
    await _send_stands_page(
        update,
        context,
        user=user,
...
async def on_search_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Handles search query input after pressing "🔎 Поиск стенда" in /status.
    """
    if update.message is None or update.message.text is None:
        return
    if not context.user_data.get("search_pending"):
        return
    context.user_data["search_pending"] = False
    query = update.message.text.strip()
    if not query:
        await update.message.reply_text("Пустой запрос. Откройте /status и попробуйте снова.")
        return
    context.user_data["last_search_query"] = query
    # Reuse /search implementation by faking a command call would be messy; call store directly.
    store: SQLiteStore = context.application.bot_data["store"]
    admin_ids: set[int] = context.application.bot_data["admin_ids"]
...
def main() -> None:
    ...
    app.add_handler(CallbackQueryHandler(on_status_callback, pattern=r"^status:"))
    app.add_handler(CallbackQueryHandler(on_search_nav_callback, pattern=r"^search:p:"))
    app.add_handler(CallbackQueryHandler(on_stand_callback, pattern=r"^stand:\d+:(take|free)$"))

    # Reply-keyboard safety net
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, on_search_text))
